"""
Extract the bibliometric population for one OpenAlex topic.

Reads:  nothing (queries the OpenAlex /works endpoint directly).
Writes: affil_full.csv     one row per author-affiliation pair, with paper id,
                           title, year, citation count and OpenAlex's own
                           standardised institution names;
        addr_uniq_full.csv the deduplicated raw affiliation strings that step 02
                           has to geocode.
Set before running: MAILTO, TOPIC_ID, the publication-year window and
max_records (the cursor stops early once the topic is exhausted).
"""
...
def fetch_works_by_topic(topic_id, year_from=2018, year_to=2024, max_records=60000):
    ...
    while len(works) < max_records:
        ...
        if not cursor:
            print("cursor exhausted (whole population retrieved), stopping early.")
            break
        time.sleep(0.2)
