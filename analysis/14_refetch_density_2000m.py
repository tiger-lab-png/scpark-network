...
UPPERBOUND_FILE = "density_5000m.csv"  # 2km 計數不得大於同座標的 5km 計數  # 每座標已知的 2,000m 計數（5km 計數的物理下界）
...
def run_batch(coords, bounds, max_attempts=12):
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if bad:
                print(f"  [資料驗證失敗：{len(bad)} 個座標的 2km 計數 > 同座標 5km 計數，"
                      f"@ {ep} 資料庫疑似區域性覆蓋，換端點]")
                mark_failed(ep)
                continue
            return totals, ep
...
def main():
    ...
    if os.path.exists(UPPERBOUND_FILE):
        ...
        print(f"已載入 {len(bounds)} 筆 5km 上界驗證資料（反向）")
    else:
        print(f"⚠ 找不到 {UPPERBOUND_FILE}，資料層驗證停用（不建議）")
